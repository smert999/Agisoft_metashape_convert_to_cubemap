...
def convert_spherical_to_cubemap(spherical_image_path, output_folder, camera_label, persp_size=None, overlap=10, 
    ...
    def process_face(face_name, params):
        try:
            ...
            # --- Явная очистка больших переменных ---
            ...
            # --- Очистка в случае ошибки (если переменные были созданы) ---
...
if 'PyQt5' in sys.modules:
    ...
    class ProcessCamerasThread(QThread):
        ...
        def __init__(self, cameras, output_folder, options):
            ...
            # Количество потоков для параллельной обработки РАЗНЫХ камер 
            # Используем значение из options, с fallback на половину CPU или 1
            default_camera_threads = max(1, (os.cpu_count() // 2) if os.cpu_count() else 1) # Более надежный fallback
                    ...
                    for result in conversion_results: # Итерируем по успешно сконвертированным
                        ...
                        try:
                            ...
                            self.update_progress.emit(
                             ...
                             # --- Принудительный сбор мусора после обработки камеры ---
                             gc.collect()
                             # -------------------------------------------------------
...
if 'PyQt5' in sys.modules:
    ...
    class CubemapConverterGUI(QMainWindow):
        ...
        def init_ui(self):
            ...
            # --- Добавляем настройку потоков для камер ---
            camera_thread_layout = QHBoxLayout()
            camera_thread_label = QLabel(_("camera_threads_label")) # Новый текст
            self.camera_thread_spinner = QSpinBox()
            # Устанавливаем диапазон и значение по умолчанию (например, половина ядер, но не меньше 1)
            ...
            self.camera_thread_spinner.setToolTip(_("camera_threads_tooltip")) # Новый тултип
        ...
        def start_processing(self):
            ...
            try:
                ...
                # Обновляем options в GUI перед созданием потока
                options = {
                    ...
                    "camera_threads": self.camera_thread_spinner.value(), # Добавляем значение из нового спиннера
                    "selected_faces": selected_faces,
                ...
                self.process_thread = ProcessCamerasThread(
                    ...
                    options=options # Передаем обновленные options
                )
                ...
                # Обновляем статус бар, включая оба значения потоков
                faces_threads = self.thread_spinner.value() # Получаем значение потоков граней
                camera_threads = self.camera_thread_spinner.value() # Получаем значение потоков камер
                self.status_bar.showMessage(_("processing_info").format(len(spherical_cameras), f"{camera_threads}/{faces_threads}")) # Обновленный формат
